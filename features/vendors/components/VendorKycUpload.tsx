...
import { Vendor, SubmitKycDto, KycReviewDto } from "../types";
import { KYC_DOCS, KycStatus } from "../constants";
import {
...
export const VendorKycUpload: React.FC<VendorKycUploadProps> = ({
  ...
  useEffect(() => {
    if (vendor.verification) {
      ...
      Object.entries(vendor.verification).forEach(([key, ok]) => {
        initial[key] = ok ? "verified" : "rejected";
      });
      // For any docs in KYC_DOCS not in verification, default to pending
      KYC_DOCS.forEach((d) => {
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const path = await uploadToStorage(vendor.id, docKey, file);

      // 3. Finalize KYC with backend (Partial Update)
      await uploadVendorKycByAdmin(vendor.id, {
  ...
  const handleViewDocument = async (docKey: string) => {
    const docPath =
      vendor.kyc?.documents?.[docKey as keyof typeof vendor.kyc.documents] ||
      (vendor as any).documents?.[docKey];
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      ...
                      className="w-7 h-7 rounded-lg flex items-center justify-center border border-gray-200 text-gray-400 hover:border-[#8a9e60] hover:text-[#8a9e60] transition-colors"
                      title="View Document"
                    >
