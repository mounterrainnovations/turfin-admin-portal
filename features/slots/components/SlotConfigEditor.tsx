...
import { Clock, Calendar, Warning } from "@phosphor-icons/react";
...
export const SlotConfigEditor: React.FC<SlotConfigEditorProps> = ({
  ...
  const currentDayConfig = config.dailyConfigs?.find(
    (p) => p.dayOfWeek === activeDay,
  ) || {
    dayOfWeek: activeDay,
    openTime: "06:00",
    closeTime: "22:00",
  ...
  const handleDayChange = (
    field: "openTime" | "closeTime" | "pricePaise",
    value: string | number,
  ) => {
    ...
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
          ...
          <div className="flex items-center gap-2 bg-gray-50 p-1.5 rounded-xl border border-gray-200">
            <div className="relative">
              <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-600 text-[10px]">
                ₹
              </span>
              <input
                type="number"
                ...
                className="w-24 pl-5 pr-2 py-1.5 text-xs text-gray-900 bg-white border border-gray-200 rounded-lg focus:outline-none focus:border-[#8a9e60]"
              />
            </div>
            <button
              onClick={() => {
                ...
                const newDailyConfigs = config.dailyConfigs.map(
                  (dayConfig) => ({
                    ...dayConfig,
              ...
              disabled={!bulkPrice}
              className="px-3 py-1.5 bg-[#8a9e60] text-white text-[11px] font-bold rounded-lg hover:opacity-90 disabled:opacity-50 transition-opacity"
            >
              APPLY ALL DAYS
            </button>
          </div>
      ...
      <div className="px-4 py-3 bg-amber-50 border-t border-amber-100/50 flex items-start gap-2">
        ...
        <p className="text-[10px] text-amber-700 leading-relaxed">
          Changes here only update the <b>Slot Configuration</b>. To generate
          physical slots for booking, visit the <b>Schedule</b> tab after
          onboarding.
        </p>
      </div>
    </div>
